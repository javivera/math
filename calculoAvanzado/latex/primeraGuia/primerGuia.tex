...
\begin{ej}
	...
	\begin{proof}
		$\Leftarrow )$ Si tengo $g: B \ra A$ inyectiva , se que para cada $x \in B$ existe un $y = f(x) \in A$ entonces puedo tomar la función $h: A \ra B$ dada por $h(x) = g^{-1}(x)$ si $x \in im(g)$ si no $h(x) = x$ esta es sobreyectiva , por que para cada $y \in B$ seguro tengo un único $x \in A$ tal que $g^{-1}(x) = y$, si no fuera único el $x$ entonces la función $g$ no estaría bien definida.
